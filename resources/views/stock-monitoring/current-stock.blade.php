    ...
    <section class="mt-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
	        ...
	        <div class="mt-6 overflow-x-auto">
	            <form method="GET" action="{{ route('monitoring.stok.index') }}" class="flex flex-nowrap items-end gap-3 min-w-max">
	                <input type="text" name="search" value="{{ $search }}" placeholder="Cari kode, nama, atau merek obat..." class="min-w-[260px] flex-1 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500">
	                <select name="category_id" class="w-56 shrink-0 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500">
