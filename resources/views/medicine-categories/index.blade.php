<x-app-layout>
    ...
    <section class="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form method="GET" action="{{ route('medicine-categories.index') }}" class="mt-6 grid gap-3 md:grid-cols-[1fr_auto]">
            <input
                type="text"
                name="search"
                value="{{ $search }}"
            ...
            >
            <button type="submit" class="rounded-2xl border border-slate-300 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50">
                Cari
            </button>
        </form>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    @forelse ($categories as $category)
                        <tr>
                            ...
                            <td class="px-4 py-3 text-slate-600">{{ $category->created_at->format('d M Y') }}</td>
                            <td class="px-4 py-3">
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-4 py-8 text-center text-slate-500">Belum ada data kategori obat.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
