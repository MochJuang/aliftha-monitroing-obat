    ...
    <section class="mt-6 rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
        <div>
	        ...
	        <div class="mt-6 overflow-x-auto">
	            <form method="GET" action="{{ route('activity-logs.index') }}" class="flex flex-col gap-3 md:flex-row md:flex-nowrap md:items-end min-w-max">
	                <input type="text" name="search" value="{{ $search }}" placeholder="Cari deskripsi, user, atau IP..." class="w-full min-w-0 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500 md:flex-1 md:min-w-[280px]">
	                <select name="module" class="w-full min-w-0 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500 md:w-44 md:shrink-0">
	                <option value="">Semua modul</option>
	                @foreach ($modules as $moduleOption)
	                    <option value="{{ $moduleOption }}" @selected($module === $moduleOption)>{{ $moduleOption }}</option>
	                @endforeach
	                </select>
	                <select name="action" class="w-full min-w-0 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500 md:w-44 md:shrink-0">
	                <option value="">Semua aksi</option>
	                @foreach ($actions as $actionOption)
	                    <option value="{{ $actionOption }}" @selected($action === $actionOption)>{{ $actionOption }}</option>
	                @endforeach
	                </select>
	                <select name="user_id" class="w-full min-w-0 rounded-2xl border-slate-300 text-sm shadow-sm focus:border-amber-500 focus:ring-amber-500 md:w-56 md:shrink-0">
